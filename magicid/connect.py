"""Native cognis-connect emit for magicid — forward findings to any platform.

Maps magicid's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    magicid ... --format json | magicid-emit --to stix
    magicid-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out["title"] = rec.get("true_type", "")
        out["severity"] = "info" if rec.get("severity") == "ok" else rec.get("severity", "info")
        out["type"] = "file_type_mismatch"
        out["description"] = f"Detected {rec.get('true_type', 'unknown')} instead of {rec.get('declared_ext', 'none')}"
        out["tags"] = [rec.get("magic_id", ""), rec.get("file_type", "")]
